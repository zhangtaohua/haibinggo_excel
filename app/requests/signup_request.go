...
import (
	"haibinggo/app/requests/validators"
...
// SignupUsingPhoneRequest 通过手机注册的请求信息
type SignupUsingPhoneEmailRequest struct {
	Phone           string `json:"phone,omitempty" valid:"phone"`
	Email           string `json:"email,omitempty" valid:"email"`
	Name            string `valid:"name" json:"name"`
...
func SignupUsingPhoneEmail(data interface{}, c *gin.Context) map[string][]string {
	...
	_data := data.(*SignupUsingPhoneEmailRequest)
	errs = validators.ValidatePasswordConfirm(_data.Password, _data.PasswordConfirm, errs)
